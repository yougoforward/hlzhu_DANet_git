...
__all__ = ['GLCNet4', 'get_glcnet4']


class GLCNet4(BaseNet):
    ...
    def __init__(self, nclass, backbone, aux=False, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(GLCNet4, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        self.head = GLCNet4Head(2048, nclass, norm_layer)
...
class GLCNet4Head(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer):
        super(GLCNet4Head, self).__init__()
...
def get_glcnet4(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = GLCNet4(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
