...
__all__ = ['ASGNet', 'get_asgnet']


class ASGNet(BaseNet):
    ...
    def __init__(self, nclass, backbone, aux=False, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(ASGNet, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        self.head = ASGNetHead(2048, nclass, norm_layer)
...
class ASGNetHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer):
        super(ASGNetHead, self).__init__()
...
def get_asgnet(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = ASGNet(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
