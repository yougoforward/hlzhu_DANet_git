...
__all__ = ['DANet_pam', 'get_danet_pam']


class DANet_pam(BaseNet):
    ...
    def __init__(self, nclass, backbone, aux=False, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(DANet_pam, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        self.head = DANetPAMHead(2048, nclass, norm_layer)
...
class DANetPAMHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer):
        super(DANetPAMHead, self).__init__()
...
def get_danet_pam(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = DANet_pam(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
