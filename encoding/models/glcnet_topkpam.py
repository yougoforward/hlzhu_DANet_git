...
__all__ = ['GLCNet5_topkpam', 'get_glcnet5_topkpam']


class GLCNet5_topkpam(BaseNet):
    ...
    def __init__(self, nclass, backbone, aux=False, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(GLCNet5_topkpam, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        self.head = GLCNet5_topkpamHead(2048, nclass, norm_layer)
...
class GLCNet5_topkpamHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer):
        super(GLCNet5_topkpamHead, self).__init__()
        ...
        self.conv5as = nn.Sequential(nn.Conv2d(in_channels, inter_channels, 3, padding=1, bias=False),
                                    norm_layer(inter_channels),
                                    nn.ReLU())
        self.sa = topk_PAM_Module(inter_channels, 64, inter_channels, 2)
...
def get_glcnet5_topkpam(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = GLCNet5_topkpam(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
