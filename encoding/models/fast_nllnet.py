...
from ..nn import selective_aggregation_ASPP_Module
from ..nn import nonlocal_sampling_Module
__all__ = ['fast_nllNet', 'get_fast_nllnet']


class fast_nllNet(BaseNet):
    ...
    def __init__(self, nclass, backbone, aux=False, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(fast_nllNet, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        self.head = fast_nllNetHead(2048, nclass, norm_layer)

        self.conv5a = nn.Sequential(nn.Conv2d(2048+512, 2048, 3, padding=1, bias=False),
                                    norm_layer(2048),
                                    nn.ReLU(inplace=True))
        self.nlsample = nonlocal_sampling_Module(2048, 2)
    def forward(self, x):
        imsize = x.size()[2:]
        c1, c2, c3, c4 = self.base_forward(x)
        c4_up = self.nlsample(c4)
        cat_c = torch.cat((c4_up,c2), 1)
        c4 = self.conv5a(cat_c)+c4_up
        x = self.head(c4)
        x = list(x)
...
class fast_nllNetHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer):
        super(fast_nllNetHead, self).__init__()
...
def get_fast_nllnet(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = fast_nllNet(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
